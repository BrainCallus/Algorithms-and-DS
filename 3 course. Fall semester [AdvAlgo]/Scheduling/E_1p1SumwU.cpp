...
struct Job {
    ...
    explicit Job(int id, int deadline, int lateCost) {
        this->id = id;
...
int findTotalLoss() {
    Vertex<Job *> *root = nullptr;
    sort(jobs.begin(), jobs.end(), jobCmpDeadlineAsc);
    int t = 0;
    LL totalLoss = 0;
    for (Job *job: jobs) {
        root = insert(root, job, jobNodeBuilder);
        if (t < job->deadline) {
            ++t;
        } else {
            Job *toRemove = root->priorityValue;
            totalLoss += toRemove->lateCost;
...
void constructSchedule() {
    ...
    for (Job *job: jobs) {
        schedule[job->id] = job->excluded ? t++ : ts++;
    }
    for (LL s: schedule) {
        cout << s << " ";
    }
